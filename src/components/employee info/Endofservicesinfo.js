...
const Endofservicesinfo = () => {
  ...
  const deleteRow = async () => {
    try {
      await axios.delete(`${config.baseUrl}/api/deleteEndofservice/${servicesData._id}`,
        { headers: { Authorization: `Bearer ${config.accessToken}` } }
      )
        .then(res => {
          setAlert(false)
          history.push('/Leftemployee')
        })
    } catch (error) {
      console.log(error)
    }
                ...
                <div className="card-body p-4">
                  ...
                  <div className="row pt-1">
                    <div className="col-6 mb-3">
                      <h6>Date</h6>
                      <p className="text-muted">{data?.date ? moment.parseZone(data.date).local().format("DD/MM/YYYY") : null}</p>
                    </div>
                    <div className="col-6 mb-3">
                      <h6>Exit Type</h6>

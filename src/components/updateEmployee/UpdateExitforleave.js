...
const UpdateExitforleave = ({
  ...
  useEffect(() => {
    ...
    if (update.lastNumberOfDayLeave)
  ...
  const updateRow = async ({ action }) => {
    ...
    try {
      await axios.put(
        `${config.baseUrl}/api/updateExitofleave/${update?._id}`,
        obj,
        { headers: { Authorization: `Bearer ${config.accessToken}` } }
      );
      getEmployeeByIdExitLeave();
  ...
  // Calculate Day startDate and endDate
  useEffect(() => {
    const start = leaveStartDate || update.leaveStartDate;
    const end = leaveEndDate || update.leaveEndDate;
    if (start && end) {
    ...
    leaveStartDate,
    leaveEndDate,
    update.leaveStartDate,
    update.leaveEndDate,
  ]);
  
  // Calculate Day startDate and endDate
  useEffect(() => {
    const start = lastLeaveStartDate || update.lastLeaveStartDate;
    const end = lastLeaveEndDate || update.lastLeaveEndDate;
    if (start && end) {
      const diff = dayjs(end).diff(dayjs(start), "day") + 1;
      setLastNumberOfLeave(diff);
    }
  }, [
    lastLeaveStartDate,
    lastLeaveEndDate,
    update.lastLeaveStartDate,
    update.lastLeaveEndDate,
  ]);   


  // Function to check if the employee is eligible for leave
  const checkEligible = async () => {
    ...
    try {
      const result = await axios.get(
        `${config.baseUrl}/api/CheckEligibleEmployee/${empId}`,
        {
          headers: { Authorization: `Bearer ${config.accessToken}` },
        }
      );
        ...
        <Dialog open={showDialog} fullWidth maxWidth="lg">
          ...
          <DialogContent>
            <div className="container">
              <h1 className="mt-3 title text-center">update Exit For Leave</h1>

...
const getRowClassName = (params) => {
  ...
  const IdmonthsDifference = (expiryDate.getFullYear() - currentDate.getFullYear()) * 12 +
    expiryDate.getMonth() - currentDate.getMonth();


  const passportmonthsDifference = (PassportexpiryDate.getFullYear() - currentDate.getFullYear()) * 12 +
  PassportexpiryDate.getMonth() - currentDate.getMonth();

  if (IdmonthsDifference <= 2 && IdmonthsDifference >= 0) {
    return 'expiry-id-row'; // CSS class name for highlighting
  }


  if (IdmonthsDifference <= 2 && IdmonthsDifference >= 0) {
    if (expiryDate.getTime() === PassportexpiryDate.getTime()) {
      console.log('Both dates are the same. Return "expiry-passport-id-row"');
      ...
      console.log('Dates are different. Return "expiry-id-row"');
  ...
  if (passportmonthsDifference <= 2 && passportmonthsDifference >= 0) {
    return 'passport-id-row'; // CSS class name for highlighting
  }
